...
class SubjectSerializer(serializers.ModelSerializer):
    ...
    study_start_time = serializers.TimeField(required=False, default="09:00:00")
    study_end_time   = serializers.TimeField(required=False, default="22:00:00")
    ...
    def validate_syllabus_completion(self, value):
        if not (0 <= value <= 100):
            raise serializers.ValidationError("Syllabus completion must be between 0 and 100.")
        return value
    ...
    def validate(self, attrs):
        ...
        start = attrs.get("study_start_time")
        end   = attrs.get("study_end_time")
        if start and end and start >= end:
            raise serializers.ValidationError(
                {"study_end_time": "End time must be after start time."}
            )
        return attrs
